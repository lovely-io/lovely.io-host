...
describe Document do
  describe "validation" do
    before do
      ...
      @document = FactoryGirl.build(:document, :version_id => 1)
    end
    ...
    it "should fail with a non-uniq path" do
      FactoryGirl.create(:document, :version_id => @document.version_id, :path => @document.path)
      @document.should have(1).error_on(:path)
    end

    it "should pass if the path is used in a different version" do
      FactoryGirl.create(:document, :version_id => @document.version_id + 1, :path => @document.path)
      @document.should be_valid
    end
